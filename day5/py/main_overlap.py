    ...
    for map in maps:
        ...
        while (len(top_ranges) > 0):
            ...
            for map_lo, map_hi, slide in map:
                overlap_start = max(rg_lo, map_lo)
                overlap_end = min(rg_hi, map_hi)
                ...
                if overlap_start < overlap_end:
                    ...
                    if rg_lo < overlap_start:
                        top_ranges.append([rg_lo, overlap_start])
                
                    if overlap_end < rg_hi:
                        top_ranges.append([overlap_end, rg_hi])
